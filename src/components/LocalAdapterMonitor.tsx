...
export default function LocalAdapterMonitor() {
  ...
  const exportAsJSON = () => {
    ...
    const dataStr = JSON.stringify({
      ...
      adapters: adapters,
      discoveredMikrotikNeighbors: discoveredDevices,
      events: logs
    }, null, 2);
    ...
    link.download = `adapter_telemetry_snapshot_${dateStr}.json`;
    ...
    setLogs(prev => [
      `[${timestamp}] [DOCUMENTATION] Exported JSON telemetry snapshot.`,
      ...prev
    ]);
  };
  ...
  const exportAsTextReport = () => {
    ...
    let report = `${border}\n`;
    report += ` MIKROTIK ADAPTERS & NEIGHBOR DISCOVERY SPECIFICATION REPORT\n`;
    report += ` Generated on: ${dateStr}\n`;
    report += ` Connection Status: ${agentConnected ? "REAL LOCAL DAEMON CONNECTED" : "SIMULATION MODE ACTIVE"}\n`;
    report += `${border}\n\n`;

    report += `1. DETECTED PHYSICAL & VIRTUAL INTERFACES (${adapters.length} Interfaces)\n`;
    report += `${subBorder}\n`;
    
    adapters.forEach((adapter, idx) => {
      report += `[Interface #${idx + 1}] ${adapter.name}\n`;
      report += `  - Type: ${adapter.type.toUpperCase()}\n`;
      report += `  - Link Status: ${adapter.status.toUpperCase()}\n`;
      report += `  - Speed: ${adapter.speed}\n`;
      report += `  - Hardware MAC: ${adapter.mac}\n`;
      report += `  - IPv4 Address: ${adapter.ipv4}\n`;
      report += `  - Default Gateway: ${adapter.gateway}\n`;
      report += `  - DHCP Server: ${adapter.dhcpServer}\n`;
      report += `  - DNS Servers: ${adapter.dns}\n`;
      report += `  - Controller Manufacturer: ${adapter.manufacturer}\n`;
      report += `  - Chipset Hardware: ${adapter.chipset}\n`;
      report += `\n`;
    });

    report += `2. DISCOVERED MIKROTIK NEIGHBORS (CDP/MNDP Protocol)\n`;
    report += `${subBorder}\n`;
    if (discoveredDevices.length > 0) {
      discoveredDevices.forEach((dev, idx) => {
        report += `[Neighbor #${idx + 1}] ${dev.identity} (${dev.boardName})\n`;
        report += `  - Router IP: ${dev.ip}\n`;
        report += `  - Router MAC: ${dev.mac}\n`;
        report += `  - RouterOS Version: ${dev.version}\n`;
        report += `  - Gateway Uptime: ${dev.uptime}\n`;
        report += `  - Inlet Port Bind: ${dev.interfaceName}\n`;
        report += `\n`;
      });
    } else {
      report += `  No active network neighbors or MikroTik nodes discovered.\n\n`;
    }

    report += `3. DIAGNOSTIC LINK STATUS LOG STREAM\n`;
    report += `${subBorder}\n`;
    logs.slice(0, 20).forEach(log => {
      report += `  ${log}\n`;
    });
    if (logs.length > 20) {
      report += `  ... and ${logs.length - 20} more log entries.\n`;
    }
    report += `\n${border}\n`;
    report += ` End of Network Documentation Report\n`;
    report += `${border}\n`;
    ...
    link.download = `network_spec_documentation_${dateFileStr}.txt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    ...
    setLogs(prev => [
      `[${timestamp}] [DOCUMENTATION] Exported formatted network spec report (.txt).`,
      ...prev
    ]);
  };
      ...
      {activeTab === 'monitor' && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          ...
          <div className="xl:col-span-4 space-y-6">
            ...
            {/* Export and Documentation Hub */}
            <div className="bg-[#0a0a0f] border border-[#141422] p-5 rounded-2xl space-y-4">
              <div className="border-b border-zinc-900 pb-2">
                <span className="text-[9px] font-black uppercase tracking-widest text-cyan-400">Documentation & Archiving</span>
                <h3 className="text-xs font-bold text-white uppercase tracking-wider mt-0.5">Export Network State</h3>
              </div>
              <p className="text-[11px] text-zinc-400 leading-relaxed">
                Save a point-in-time snapshot of the detected physical adapters, speeds, IPs, and discovered network neighbor routers.
              </p>
              
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={exportAsJSON}
                  className="bg-[#10101b] border border-zinc-800 hover:border-cyan-500 hover:bg-cyan-950/10 text-white text-xs font-bold py-3 px-3 rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 group cursor-pointer"
                >
                  <FileJson size={20} className="text-cyan-400 group-hover:scale-110 transition-transform" />
                  <div>
                    <span className="block font-bold text-[11px]">Export JSON</span>
                    <span className="block text-[8px] text-zinc-500 font-normal mt-0.5">Full JSON Payload</span>
                  </div>
                </button>

                <button
                  onClick={exportAsTextReport}
                  className="bg-[#10101b] border border-zinc-800 hover:border-emerald-500 hover:bg-emerald-950/10 text-white text-xs font-bold py-3 px-3 rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 group cursor-pointer"
                >
                  <FileText size={20} className="text-emerald-400 group-hover:scale-110 transition-transform" />
                  <div>
                    <span className="block font-bold text-[11px]">Export Report</span>
                    <span className="block text-[8px] text-zinc-500 font-normal mt-0.5">Formatted TXT/MD</span>
                  </div>
                </button>
              </div>
            </div>
